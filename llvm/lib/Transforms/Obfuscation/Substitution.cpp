...
struct Substitution : public FunctionPass {
  ...
  Substitution() : FunctionPass(ID) { initFunctions(); }
...
// Tanner
BinaryOperator *Substitution::CreateFNeg(Value *Op, const Twine &Name,
                                         Instruction *InsertBefore) {
  Value *zero = ConstantFP::getZeroValueForNegation(Op->getType());
  return BinaryOperator::CreateFSub(zero, Op, Name, InsertBefore);
}
